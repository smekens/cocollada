...
coco_index_of_refraction_t *coco_index_of_refraction_parse(coco_ctx_t *ctx, yaxp_node_t *node0)
{
	coco_index_of_refraction_t *result = coco_ctx_factory(ctx, coco_index_of_refraction_t);
...
void coco_index_of_refraction_dump(coco_ctx_t *ctx, coco_index_of_refraction_t *index_of_refraction, int indent)
{
	...
	COCO_DUMP_INDENT(indent, "Index of refraction:\n");

	indent++;

	coco_ctx_dump(ctx, coco_fx_common_float_t, index_of_refraction->flt, indent);
