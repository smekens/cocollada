...
coco_transparency_t *coco_transparency_parse(coco_ctx_t *ctx, yaxp_node_t *node0)
{
	coco_transparency_t *result = coco_ctx_factory(ctx, coco_transparency_t);
...
void coco_transparency_dump(coco_ctx_t *ctx, coco_transparency_t *transparency, int indent)
{
	...
	COCO_DUMP_INDENT(indent, "Transparency:\n");

	indent++;

	coco_ctx_dump(ctx, coco_fx_common_float_t, transparency->flt, indent);
