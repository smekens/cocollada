...
collada_specular_t *collada_specular_parser(collada_ctx_t *ctx, yaxp_node_t *node0)
{
	collada_specular_t *result = collada_ctx_factory(ctx, collada_specular_t);
...
void collada_specular_dump(collada_ctx_t *ctx, collada_specular_t *specular, int indent)
{
	if(specular == NULL) {
		return;
	}
	...
	collada_ctx_dump(ctx, collada_color_t, specular->color, indent);
