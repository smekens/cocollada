...
collada_ambient_fx_t *collada_ambient_fx_parser(collada_ctx_t *ctx, yaxp_node_t *node0)
{
	collada_ambient_fx_t *result = collada_ctx_factory(ctx, collada_ambient_fx_t);
...
void collada_ambient_fx_dump(collada_ctx_t *ctx, collada_ambient_fx_t *ambient_fx, int indent)
{
	if(ambient_fx == NULL) {
		return;
	}
	...
	collada_ctx_dump(ctx, collada_color_t, ambient_fx->color, indent);
